...
onMounted(() => {
	if (mapRef.value && latitude.value && longitude.value) {
		const map = $leaflet
			.map(mapRef.value)
...
<template>
    ...
    :class="reverse ? 'xl:flex-row-reverse' : 'xl:flex-row'"
  >
		<!-- map -->
		<div
			class="card__map w-full h-80 overflow-hidden -z-1"
			ref="mapRef"
		></div>

		<div
			class="card__body w-full text-center md:text-left px-6 py-20 bg-pale xl:flex-1"
		>
			<p
			...
			>
				...
				>
					<slot name="address">
						...
						><br />
						3886 Wellington Street <br />Toronto,
						Ontario M9C 3J5
					</slot>
				</li>
				<li
